#define ThirteenLayersAnalysis_cxx
#include "ThirteenLayersAnalysis.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
void ThirteenLayersAnalysis::Loop()
{   
   // CALO ANALYSIS 
   if (fChain == 0) return;
   ...
   // HISTOGRAMS           
   // INVARAINT MASS - e+e- 
   TH1F *InvariantMass = new TH1F("invariantMass","Reconstructed Invariant Mass with 2 clusters (13 Layers); Mass [GeV]; Events",150,0,115);
   TH1F *massHisto = new TH1F("massHisto","Sum of two most Energetic Clusters (13 Layers); Mass [GeV]; Events",150,0,115);        
   ...
   // cos theta
   TH1F *costheta = new TH1F("cos_theta","Cos theta (13 Layers); Cos theta; ",100,-1,1);  
   // 3 cluster Invariant Mass
   TH1F *InvariantMass3 = new TH1F("invariantMass3","Reconstructed Invariant Mass [3rd cluster > 10 GeV] (13 Layers); Mass [GeV]; Events",150,0,115);
   ...
   // write to text file 
   ofstream NewerFile("InvariantMassFit_13Layers.txt");
   NewerFile << mean2 << " " << mean_error2 << " " << sigma2 << " " << sigma_error2 << "\n\n";
   NewerFile.close();
   c1->SaveAs("InvariantMassFit_13Layers.pdf");
   ...
   c5->SaveAs("cosTheta_13layers.pdf");
   ...
   // write to text file 
   ofstream NewestFile("InvariantMassFit_13Layers_3ClustersBound.txt");
   NewestFile << mean3 << " " << mean_error3 << " " << sigma3 << " " << sigma_error3 << "\n\n";
   NewestFile.close();
   c6->SaveAs("InvariantMassFit_13Layers_3ClustersBound.pdf");
