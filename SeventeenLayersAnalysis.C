#define SeventeenLayersAnalysis_cxx
#include "SeventeenLayersAnalysis.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
void SeventeenLayersAnalysis::Loop()
{   
   // CALO ANALYSIS 
   if (fChain == 0) return;
   ...
   // HISTOGRAMS           
   // INVARAINT MASS - e+e- 
   TH1F *InvariantMass = new TH1F("invariantMass","Reconstructed Invariant Mass with 2 clusters (17 Layers); Mass [GeV]; Events",150,0,115);
   TH1F *massHisto = new TH1F("massHisto","CaloCluster Energies Sum (17 Layers); Mass [GeV]; Events",150,0,115);        
   ...
   // cos theta
   TH1F *costheta = new TH1F("cos_theta","Cos theta 12 (17 Layers); Cos theta; ",100,-1,1);  
   // 3 cluster Invariant Mass
   TH1F *InvariantMass3 = new TH1F("invariantMass3","Reconstructed Invariant Mass [3rd cluster > 10 GeV] (17 Layers); Mass [GeV]; Events",150,0,115);
   ...
   for (Long64_t jentry=0; jentry<nEvents; jentry++) {
      ...
      //////////////// 2 CLUSTERS /////////////////////////
      // invariant mass 
      invariantMass = sqrt(2 * CaloClusters_core_energy[0] * CaloClusters_core_energy[1] * (1 - cosTheta12));
      std::cout << "Invariant Mass: " << invariantMass << "\n";
      // plot invariant mass for 2 most energtic clusters
      if(CaloClusters_ > 1){
      ...
      // invariant mass
      invariantMass3 = sqrt(E1E2 + E1E3 + E2E3);
      std::cout << "Invariant Mass (3 clusters): " << invariantMass3 << "\n";
      // 2 clusters 
      if(CaloClusters_ == 2){
   ...
   // write to text file 
   ofstream NewerFile("InvariantMassFit_17Layers.txt");
   NewerFile << mean2 << " " << mean_error2 << " " << sigma2 << " " << sigma_error2 << "\n\n";
   NewerFile.close();
   InvariantMass->SetDirectory(0);
   c1->SaveAs("InvariantMassFit_17Layers.pdf");
   ...
   // write to text file 
   ofstream MyFile("ClusterEnergySum_17Layers.txt");
   ...
   c2->SaveAs("ClusterEnergySum_14Layers.pdf");
   ...
   c6->SaveAs("cosTheta_17layers.pdf");
   ...
   // write to text file 
   ofstream NewestFile("InvariantMassFit_17Layers_3ClustersBound.txt");
   NewestFile << mean3 << " " << mean_error3 << " " << sigma3 << " " << sigma_error3 << "\n\n";
   NewestFile.close();
   c7->SaveAs("InvariantMassFit_17Layers_3ClustersBound.pdf");
