#define TenLayersAnalysis_cxx
#include "TenLayersAnalysis.h"
...
{   
   ...
   // HISTOGRAMS           
   // INVARAINT MASS - e+e- 
   TH1F *InvariantMass = new TH1F("invariantMass","Reconstructed Invariant Mass with 2 clusters (10 Layers); Mass [GeV]; Events",150,0,115);
   // Sum of two most energetic clutsers 
   TH1F *massHisto = new TH1F("massHisto","Sum of two most Energetic Clusters (10 Layers); Mass [GeV]; Events",150,0,115);        
   gStyle->SetOptStat("ne");                                
   // PSEUDORAPIDITY    
   // pseudorapidity for events that are forming clutsers
   TH1F *PseudoWith = new TH1F("PseudoWith","e+e- Pseudorapidity (with clusters); Pseudorapidity; Events", 100,-1,1);           
   // pseudorapidity for events that are not forming clutsers 
   TH1F *PseudoWithout = new TH1F("PseudoWithout","e+e- Pseudorapidity (without clusters); Pseudorapidity; Events", 100,-5,5);   
   // cos theta
   TH1F *costheta = new TH1F("cos_theta","Cos theta (10 Layers); Cos theta; ",100,-1,1);  
   // 3 cluster Invariant Mass
   TH1F *InvariantMass3 = new TH1F("invariantMass3","Reconstructed Invariant Mass [3rd cluster > 10 GeV] (10 Layers); Mass [GeV]; Events",150,0,115);
   ...
   for (Long64_t jentry=0; jentry<nEvents; jentry++) {
      ...
      if(CaloClusters_ > 2){
         ...
         // if third clutser energy > 10 GeV use three clutser invariant mass
         if(CaloClusters_core_energy[2] > 10){
           InvariantMass3->Fill(invariantMass3);
         }
         // if third clutser energy < 10 GeV use two cluster invariant mass 
         if(CaloClusters_core_energy[2] < 10){
            InvariantMass3->Fill(invariantMass);
         }
      } 
      ...
      // counting events with no clusters 
      if(CaloClusters_==0){
         Events = Events +1;
      }
   ...
   // write to text file 
   ofstream NewerFile("InvariantMassFit_10Layers.txt");
   NewerFile << mean2 << " " << mean_error2 << " " << sigma2 << " " << sigma_error2 << "\n\n";
   NewerFile.close();
   c1->SaveAs("InvariantMassFit_10Layers.pdf");
   ...
   // CLUSTER ENERGIES 
   TCanvas * c5 = new TCanvas(); costheta->Draw();
   c5->SaveAs("cosTheta_10layers.pdf");
   costheta->SetDirectory(0);

   // INVARIANT MASS - 3 clusters 
   TCanvas * c6 = new TCanvas(); InvariantMass3->Draw();
   // Fit peak 
   ...
   // write to text file 
   ofstream NewestFile("InvariantMassFit_10Layers_3ClustersBound.txt");
   NewestFile << mean3 << " " << mean_error3 << " " << sigma3 << " " << sigma_error3 << "\n\n";
   NewestFile.close();
   c6->SaveAs("InvariantMassFit_10Layers_3ClustersBound.pdf");
