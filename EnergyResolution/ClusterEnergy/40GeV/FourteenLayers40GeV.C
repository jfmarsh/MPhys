#define FourteenLayers40GeV_cxx
#include "FourteenLayers40GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 40 GeV Electron Beam (14 Layers); Energy [GeV]; #Events",150,0,50);    
   ...
   gStyle->SetStatX(0.5);
   // Fit peak 
   TF1 *fit = new TF1("fit","gaus",30,50);
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_40GeV_14Layers.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_40GeV_14layers.pdf");
