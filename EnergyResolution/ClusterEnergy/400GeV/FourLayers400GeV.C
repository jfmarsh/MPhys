#define FourLayers400GeV_cxx
#include "FourLayers400GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 400 GeV Electron Beam (4 Layers); Energy [GeV]; #Events",150,0,420);    
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_400GeV_4Layers_BOUND.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_400GeV_4layers_BOUND.pdf");
