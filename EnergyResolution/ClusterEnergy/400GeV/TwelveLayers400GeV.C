#define TwelveLayers400GeV_cxx
#include "TwelveLayers400GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 400 GeV Electron Beam (12 Layers); Energy [GeV]; #Clusters",150,0,420);    
   ...
   gStyle->SetStatX(0.5);
   // Fit peak 
   TF1 *fit = new TF1("fit","gaus",384,415);
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_400GeV_12Layers.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_400GeV_12layers.pdf");
