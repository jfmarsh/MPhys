#define FourLayers50GeV_cxx
#include "FourLayers50GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 50 GeV Electron Beam (4 Layers); Energy [GeV]; #Clusters",150,0,60);    
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_50GeV_4Layers.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_50GeV_4layers.pdf");
