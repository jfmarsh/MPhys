#define EightLayers50GeV_cxx
#include "EightLayers50GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy","Cluster Energy for 50 GeV Electron Particle Gun (8 Layers); Energy [GeV]; Events",150,0,60);    
   ...
   for (Long64_t jentry=0; jentry<nEvents; jentry++) {
      ...
      {// loop over CaloClusters
         ...
         if(CaloClusters_ > 1){
            if(CaloClusters_core_energy[0] < 48){
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_50GeV_8Layers.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_50GeV_8layers.pdf");
