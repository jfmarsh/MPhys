#define FourteenLayers300GeV_cxx
#include "FourteenLayers300GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
void FourteenLayers300GeV::Loop()
{  // CALO ANALYSIS 
   if (fChain == 0) return;
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 300 GeV Electron Beam (14 Layers); Energy [GeV]; #Events",150,0,350);    
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_300GeV_14Layers.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_300GeV_14layers.pdf");
