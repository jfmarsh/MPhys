#define EightLayers300GeV_cxx
#include "EightLayers300GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 300 GeV Electron Beam (8 Layers); Energy [GeV]; #Events",150,0,320);    
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_300GeV_8Layers.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_300GeV_8layers.pdf");
