#define EightLayers800GeV_cxx
#include "EightLayers800GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 800 GeV Electron Beam (8 Layers); Energy [GeV]; #Events",150,400,850);    
   gStyle->SetOptStat("ne");   
   TH1F *EnergyPeak = new TH1F("EnergyPeak"," CaloCluster Energy for 800 GeV Electron Beam (8 Layers); Energy [GeV]; #Events",150,400,850);                                 
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_800GeV_8Layers_BOUND.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_800GeV_8layers_BOUND.pdf");
   ...
   TF1 *total = new TF1("total", "gaus(0)+gaus(3)",720, 840);
   gStyle->SetOptFit(1011);
   ...
   // write to text file 
   ofstream File1("ClusterEnergyFit_800GeV_8Layers_gauss1.txt");
   ...
   // write to text file 
   ofstream File2("ClusterEnergyFit_800GeV_8Layers_gauss2.txt");
   File2 << mean2 << " " << mean_error2 << " " << sigma2 << " " << sigma_error2 << "\n\n";
   File2.close();

   ofstream File3("ClusterEnergyFit_800GeV_8Layers_totalGauss.txt");
   ...
   c2->SaveAs("ClusterEnergyFit_800GeV_8layers_2Gauss.pdf");
