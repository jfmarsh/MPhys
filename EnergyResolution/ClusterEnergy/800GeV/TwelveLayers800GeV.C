#define TwelveLayers800GeV_cxx
#include "TwelveLayers800GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 800 GeV Electron Beam (12 Layers); Energy [GeV]; #Clusters",150,400,850);    
   ...
   gStyle->SetStatX(0.5);
   // Fit peak 
   TF1 *fit = new TF1("fit","gaus",750,840);
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_800GeV_12Layers_newBOUND.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_800GeV_12layers_newBOUND.pdf");
