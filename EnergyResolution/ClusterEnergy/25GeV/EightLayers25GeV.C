#define EightLayers25GeV_cxx
#include "EightLayers25GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 25 GeV Electron Beam (8 Layers); Energy [GeV]; #Clusters",150,0,30);    
   ...
   gStyle->SetStatX(0.5);
   // Fit peak 
   TF1 *fit = new TF1("fit","gaus",23,27);
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_25GeV_8Layers_BOUND.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_25GeV_8layers_BOUND.pdf");
