#define FifteenLayers25GeV_cxx
#include "FifteenLayers25GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy","CaloCluster Energy for 25 GeV Electron Beam (15 Layers); Energy [GeV]; #Events",150,0,30);    
   ...
   gStyle->SetStatX(0.5);
   // Fit peak 
   TF1 *fit = new TF1("fit","gaus",22,28);
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_25GeV_15Layers.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_25GeV_15layers.pdf");
