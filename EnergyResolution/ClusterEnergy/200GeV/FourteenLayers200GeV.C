#define FourteenLayers200GeV_cxx
#include "FourteenLayers200GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
void FourteenLayers200GeV::Loop()
{  // CALO ANALYSIS 
   if (fChain == 0) return;
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 200 GeV Electron Beam (14 Layers); Energy [GeV]; #Events",150,0,250);    
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_200GeV_14Layers.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_200GeV_14layers.pdf");
