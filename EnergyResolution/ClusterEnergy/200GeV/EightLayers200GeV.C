#define EightLayers200GeV_cxx
#include "EightLayers200GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 200 GeV Electron Beam (8 Layers); Energy [GeV]; #Clusters",150,0,220);    
   ...
   gStyle->SetStatX(0.5);
   // Fit peak 
   TF1 *fit = new TF1("fit","gaus",192,208);
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_200GeV_8Layers_BOUND.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_200GeV_8layers_BOUND.pdf");
