#define TwelveLayers200GeV_cxx
#include "TwelveLayers200GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 200 GeV Electron Beam (12 Layers); Energy [GeV]; #Clusters",150,0,220);    
   ...
   for (Long64_t jentry=0; jentry<nEvents; jentry++) {
      ...
      {// loop over CaloClusters
         ...
         if(CaloClusters_ > 1){
            if(CaloClusters_core_energy[0] < 198){
   ...
   gStyle->SetStatX(0.5);
   // Fit peak 
   TF1 *fit = new TF1("fit","gaus",192,207);
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_200GeV_12Layers_BOUND.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_200GeV_12layers_BOUND.pdf");
