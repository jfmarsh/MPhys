#define FourteenLayers350GeV_cxx
#include "FourteenLayers350GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
void FourteenLayers350GeV::Loop()
{  // CALO ANALYSIS 
   if (fChain == 0) return;
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 350 GeV Electron Beam (14 Layers); Energy [GeV]; #Events",150,0,400);    
   ...
   gStyle->SetStatX(0.5);
   // Fit peak 
   TF1 *fit = new TF1("fit","gaus",335,360);
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_350GeV_14Layers.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_350GeV_14layers.pdf");
