#define FifteenLayers350GeV_cxx
#include "FifteenLayers350GeV.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
{  // CALO ANALYSIS 
   ...
   // HISTOGRAMS           
   // Cluster Energy
   TH1F *ClusterEnergy = new TH1F("clusterEnergy"," CaloCluster Energy for 350 GeV Electron Beam (15 Layers); Energy [GeV]; #Events",150,0,400);    
   ...
   gStyle->SetStatX(0.5);
   // Fit peak 
   TF1 *fit = new TF1("fit","gaus",335,370);
   ...
   // write to text file 
   ofstream NewerFile("ClusterEnergyFit_350GeV_15Layers.txt");
   NewerFile << mean << " " << mean_error << " " << sigma << " " << sigma_error << "\n\n";
   NewerFile.close();
   c1->SaveAs("ClusterEnergyFit_350GeV_15layers.pdf");
