#define FourteenLayersAnalysis_cxx
#include "FourteenLayersAnalysis.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
void FourteenLayersAnalysis::Loop()
{   
   // CALO ANALYSIS 
   if (fChain == 0) return;
   ...
   // HISTOGRAMS           
   // INVARAINT MASS - e+e- 
   TH1F *InvariantMass = new TH1F("invariantMass","Reconstructed Invariant Mass with 2 clusters (14 Layers); Mass [GeV]; Events",150,0,115);
   TH1F *massHisto = new TH1F("massHisto","CaloCluster Energies Sum (14 Layers); Mass [GeV]; Events",150,0,115);        
   ...
   // cos theta
   TH1F *costheta = new TH1F("cos_theta","Cos theta 12 (14 Layers); Cos theta; ",100,-1,1);  
   // 3 cluster Invariant Mass
   TH1F *InvariantMass3 = new TH1F("invariantMass3","Reconstructed Invariant Mass [3rd cluster > 10 GeV] (14 Layers); Mass [GeV]; Events",150,0,115);
   ...
   // write to text file 
   ofstream NewerFile("InvariantMassFit_14Layers.txt");
   NewerFile << mean2 << " " << mean_error2 << " " << sigma2 << " " << sigma_error2 << "\n\n";
   NewerFile.close();
   InvariantMass->SetDirectory(0);
   c1->SaveAs("InvariantMassFit_14Layers.pdf");
   ...
   // write to text file 
   ofstream MyFile("ClusterEnergySum_14Layers.txt");
   ...
   c6->SaveAs("cosTheta_14layers.pdf");
   ...
   // write to text file 
   ofstream NewestFile("InvariantMassFit_14Layers_3ClustersBound.txt");
   NewestFile << mean3 << " " << mean_error3 << " " << sigma3 << " " << sigma_error3 << "\n\n";
   NewestFile.close();
   c7->SaveAs("InvariantMassFit_14Layers_3ClustersBound.pdf");
